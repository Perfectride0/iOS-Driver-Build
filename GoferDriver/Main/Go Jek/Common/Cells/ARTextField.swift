...
class ARTextField: UIView {
    ...
    fileprivate var textMainColor : UIColor = .ThemeTextColor
    fileprivate var textHightLightColor : UIColor = .ThemeTextColor
    ...
    func setTheme() {
        if #available(iOS 12.0, *) {
            let isDarkColor = self.traitCollection.userInterfaceStyle == .dark
            self.backgroundColor = isDarkColor ? .DarkModeBackground : .SecondaryColor
        } else {
        ...
        UIView.animate(withDuration: 0.3) {
            if self.showingTitle{
                ...
                self.title.transform =  CGAffineTransform(translationX: 0,
                                                          y: -height)
                self.title.font = UIFont(name: G_BoldFont, size: 12)
                self.title.textColor = self.textMainColor.withAlphaComponent(0.5)
                self.bar.backgroundColor = self.textMainColor.withAlphaComponent(0.5)
            }
        }
    ...
    func setTitle(_ title : String,_ fontSize : CGFloat = -1){
        ...
        self.title.font = UIFont(name: G_BoldFont, size: fontSize)
    }
    func setValue(_ value : String,_ fontSize : CGFloat = -1){
        ...
        guard fontSize != -1 else{return}
        self.textField.font = UIFont(name: G_BoldFont, size: fontSize)
    }
    func setFontSize(title : CGFloat,value : CGFloat){
        self.title.font = UIFont(name: G_BoldFont, size: title)
        self.textField.font = UIFont(name: G_BoldFont, size: value)
    }

    func setHintIcon(_ img :UIImage){
        self.setIcon = true
        self.icon.setImage(img.withRenderingMode(.alwaysTemplate), for: .normal)
        self.icon.tintColor = .PrimaryColor
    ...
    func showError(_ message : String){
        ...
        UIView.animateKeyframes(withDuration: 0.5, delay: 0, options: [.calculationModeLinear], animations: {
            ...
            UIView.addKeyframe(withRelativeStartTime: 0, relativeDuration: 0.2, animations: {
                self.title.textColor = .ErrorColor
                self.bar.backgroundColor = .ErrorColor
                self.icon.tintColor = .ErrorColor
