...
class PayToGoferView: BaseView {
    ...
    var viewController: PayToGoferVC!
    ...
    override func didLoad(baseVC: BaseVC) {
        ...
        self.viewController = baseVC as? PayToGoferVC
        self.initView()
        self.darkModeChange()
    }
...
//        self.addGestureRecognizer(tap)
    ...
    func initTextSetup(){
        ...
        lblTitle.text = LangCommon.payTo.capitalized + " \(appName)"
        lbltext.text = " \(LangCommon.yourPayTo) \(appName)"
    ...
    @IBAction func payButtonTapped(_ sender :Any){
        var text = amountField.text ?? "0"
        if text == ""{
            text = "0"
        }
        guard let amount = Double(text) else {return}
        print("iam",amount)
        if amount.isZero && UserDefaults.standard.bool(forKey: "DEDUCTION"){
            self.viewController.wsToPaytoAdmin(0, payKey: nil)
        }else{
            if Shared.instance.isWebPayment{
                self.wsToWebPaytoAdmin(amount, payKey: nil)
            }else{
                if PaymentOptions.default == .stripe{
                    self.viewController.wsToPaytoAdmin(amount, payKey: nil)
                }else{
                    self.viewController.wsMethodConvetCurrency(for: amount)
                }
            }
        }
...
//        self.hideView()
    ...
    func wsToWebPaytoAdmin(_ enteredAmount : Double,payKey : String?){
        ...
        let UrlString = "\(APIBaseUrl + "api/" + APIEnums.webPayment.rawValue)?amount=\(enteredAmount)&payment_type=\(paymentType)&token=\(token)&applied_referral_amount=\(referalAmount)&pay_for=pay_to_admin&mode=\(mode)"
    ...
    func handleCurrencyConversion(response amount: Double,
        ...
        case .brainTree:
            self.viewController.authenticateBrainTreePayment(for: amount,
        ...
        case .paypal:
            ...
            //                                             for: .wallet)
            self.viewController.authenticatePaypalPayment(for: amount,
                                           enteredAmount: enteredAmount,
                                           currency: currency ?? "USD",
                                           using: key)
        default:
    ...
    func initLocalization(){
        self.payToGoferBtn.setTitle(LangCommon.payToAdmin.uppercased(),
                                    for: .normal)
        
        self.lblTitle.text = LangCommon.payToAdmin.uppercased()
        self.lbltext.text = " \(LangCommon.yourPayTo) Admin "
        self.payBtn.setTitle(LangCommon.pay.uppercased(), for: .normal)
...
extension PayToGoferView : paymentMethodSelection{
...
extension PayToGoferView {//PayPalHandlerDelegate
    func paypalHandler(didComplete paymentID: String) {
        
        guard let amount = enteredAmount else {return}
        self.viewController.wsToPaytoAdmin(amount, payKey: paymentID)
        enteredAmount = nil
    }
...
extension PayToGoferView : UITextFieldDelegate {
