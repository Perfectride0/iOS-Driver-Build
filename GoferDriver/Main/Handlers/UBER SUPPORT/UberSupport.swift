...
class UberSupport: NSObject {
    ...
    static var showingProgress = false
...
func changeStatusBarStyles(style: UIStatusBarStyle)
{
    ...
    if #available(iOS 13.0, *){
       ...
       // let app = UIApplication.shared
        ...
        if statusBar.responds(to: #selector(setter: UIView.backgroundColor)) {
            statusBar.backgroundColor = style == .default ? UIColor.SecondaryColor : .clear
        }
        UIApplication.shared.statusBarStyle = .default
    }
}
...
//MARK: Remove the progress Window
    ...
    {
//        UberSupport.showingProgress = false
        let appDelegate = UIApplication.shared.delegate as! AppDelegate
...
//MARK: Set the String size
    ...
    func getAttributedString(originalText: NSString, arrtributeText: String) -> NSMutableAttributedString
    {
        ...
        mainString.addAttribute(NSAttributedString.Key.font, value:  UIFont (name: G_RegularFont, size: 18)!, range: NSRange(location: range.location, length: arrtributeText.count))
        return mainString
    }
    ...
    func getBigAndNormalString(originalText : NSString ,normalText : NSString, attributeText : NSString , font : UIFont) -> NSMutableAttributedString
    {
        ...
        mainString.addAttribute(NSAttributedString.Key.font, value:  UIFont (name: G_RegularFont, size: 18)!, range: NSRange(location: range.location, length: attributeText.length))
        return mainString
    }
...
//MARK: Set the attributed name
    ...
    {
        let attributedString = NSMutableAttributedString(string: originalText as String, attributes: [NSAttributedString.Key.font:UIFont (name: G_RegularFont, size: fontSize)!])
...
//MARK: Set rating color
   ...
   //MERK: set attributed to  text
    func attributedText(originalText: NSString, boldText: String , fontSize : CGFloat)->NSAttributedString
    {
        ...
        let boldFontAttribute = [NSAttributedString.Key.font: UIFont (name: G_RegularFont, size: fontSize)!]
...
//MERK: attributed to converted text
    ...
    {
        let attributedString = NSMutableAttributedString(string: originalText as String, attributes: [NSAttributedString.Key.font:UIFont (name: G_RegularFont, size: fontSize)!])
        let boldFontAttribute = [NSAttributedString.Key.font: UIFont (name: G_RegularFont, size: fontSize - 4)!]
...
//MARK: Set the attributed text bold
    ...
    {
        let attributedString = NSMutableAttributedString(string: originalText as String, attributes: [NSAttributedString.Key.font:UIFont (name: G_RegularFont, size: fontSize-8)!])
...
{
    NotificationCenter.default.addObserver( self, selector: #selector(self.handleKeyboards(note:)), name: UIResponder.keyboardWillShowNotification, object: nil)
    NotificationCenter.default.addObserver( self, selector: #selector(self.handleKeyboards(note:)), name: UIResponder.keyboardWillHideNotification, object: nil)
}
//handle keyboard height dynamically
@objc func handleKeyboards( note:NSNotification )
{
    // read the CGRect from the notification (if any)
    if let keyboardFrame = (note.userInfo?[ UIResponder.keyboardFrameEndUserInfoKey ] as? NSValue)?.cgRectValue {
        if self.contentInset.bottom == 0 && keyboardFrame.height != 0 {
