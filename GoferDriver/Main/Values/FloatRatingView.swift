...
open class FloatRatingView: UIView {
    ...
    private func updateLocation(_ touch: UITouch) {
        ...
        var from1:Int = 0
        var through1:Int = maxRating-1
        var factorForCalculation:Int = +1
        if !isRTLLanguage{
            from1 = maxRating-1
            through1 = 0
            factorForCalculation = -1
        }
        for i in stride(from: from1, through: through1, by: factorForCalculation) {
            ...
            if imageView.point(inside: newLocation, with: nil) && (floatRatings || halfRatings) {
                ...
                newRating = Double(Float(i) + decimalNum)
                if halfRatings {
                    newRating = Double(Float(i) + (decimalNum > 0.75 ? 1 : (decimalNum > 0.25 ? 0.5 : 0)))
                }
            } else {
                // Whole rating
                newRating = Double(Float(i) + 1.0)
            }
            break
        }
        // Check min rating
        rating = Float(newRating < Double(minRating) ? Double(minRating) : newRating)
    ...
    override open func layoutSubviews() {
        ...
        for i in 0..<maxRating {
            var imageFrame = CGRect(x: i == (maxRating-1) ? 0 : CGFloat(4-i)*(imageXOffset+imageViewSize.width), y: 0, width: imageViewSize.width, height: imageViewSize.height)
            if !isRTLLanguage{
