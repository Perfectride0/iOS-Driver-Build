...
//  Gofer
//
//  Created by Trioangle Technologies on 31/01/19.
...
class PipeAdapter{
   ...
   static func deleteEvent(withName name : String)->Bool{
    ...
    static func deleteEvent(withID id : Int)->Bool{
        ...
        guard let index = PipeAdapter.events.firstIndex(where: { (_event) -> Bool in
...
class PipeLine{
    ...
    static func deleteEvent(withID id : Int)->Bool{
        ...
        guard let index = PipeLine.events.firstIndex(where: { (_event) -> Bool in
            return _event.id == event?.id
        }) else {return false}
