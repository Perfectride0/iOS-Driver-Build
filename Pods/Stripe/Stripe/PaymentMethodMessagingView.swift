    ...
    public override func traitCollectionDidChange(_ previousTraitCollection: UITraitCollection?) {
        ...
        Task {
            ...
            if previousTraitCollection?.isDarkMode != traitCollection.isDarkMode {
                ...
                attributedString = try await Self.makeAttributedString(from: content.display_l_html, font: configuration.font)
            }
...
extension PaymentMethodMessagingView {
    ...
    static func makeAttributedString(from html: String, font: UIFont) async throws -> NSAttributedString {
        ...
        return try await withCheckedThrowingContinuation { continuation in
            NSAttributedString.loadFromHTML(string: css + html, options: [:]) { attributedString, attributeDict, error in
                guard let attributedString = attributedString else {
                    continuation.resume(with: .failure(error ?? Error.failedToInitializeAttributedString))
                    return
                }
                // Set images to 3x
    ...
    static func loadContent(configuration: Configuration) async throws -> PaymentMethodMessagingContentResponse {
        let parameters = Self.makeMessagingContentEndpointParams(configuration: configuration)
        var request = configuration.apiClient.configuredRequest(for: APIEndpoint, additionalHeaders: [:])
        request.stp_addParameters(toURL: parameters)
        return try await withCheckedThrowingContinuation { continuation in
            configuration.apiClient.get(
                url: APIEndpoint,
                parameters: parameters) { (result: Result<PaymentMethodMessagingContentResponse, Swift.Error>) in
